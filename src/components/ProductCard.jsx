import React from 'react'
import toast from 'react-hot-toast'
import { useDispatch, useSelector } from 'react-redux'
import {add, remove} from "../redux/slice/CartSlice"

function ProductCard({data}) {
    const cart = useSelector(state => state.cart)
    const dispatch = useDispatch()


    function addToCart(){
        dispatch(add(data))
        toast.success("Added to Cart")
    }
    function removeFromCart(){
        dispatch(remove(data.id))
        toast.success("Removed from cart")
    }

  return (
    <div className="flex flex-col items-center justify-between hover:bg-white shadow-[0_3px_10px_rgb(0,0,0,0.2)]
    hover:scale-110 transition duration-300 ease-in gap-3 p-4 mt-10 ml-5 rounded-xl hover:shadow-[rgba(0,_0,_0,_0.4)_0px_30px_90px]">
        <div>
            <p className="text-gray-700 font-semibold font-serif text-lg text-left truncate w-40 mt-1 ">{data.title}</p>
        </div>
        <div>
            <p className="w-40 text-gray-400 font-normal text-[10px] text-left">{data.description.split(" ").slice(0,10).join(" ")}...</p>
        </div>
        <div className="h-[180px]">
            <img src={data.image} className="h-full w-full "/>
        </div>
        <div className="flex justify-between gap-12 items-center w-full mt-5">
            <p className="text-green-600 font-semibold">₹{data.price}</p>
            {
                cart.some((item) => item.id===data.id) ? 
                <button className="text-gray-700 border-2 border-gray-700 rounded-full font-semibold text-[12px] bg-slate-300 p-1 px-3 uppercase  hover:bg-gray-700 hover:text-white transition duration-300 ease-in" onClick={removeFromCart}>Remove Item</button> : 
                <button className="text-gray-700 border-2 border-gray-700 rounded-full font-semibold text-[12px] p-1 px-3 uppercase hover:bg-gray-700 hover:text-white transition duration-300 ease-in" onClick={addToCart}>Add to Cart</button>
            }
        </div>
            
    </div>
  )
}

export default ProductCard